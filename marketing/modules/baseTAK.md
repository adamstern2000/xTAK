...
### 2. Be the authoritative SA source for the deployment
...
- **Per-COT broadcast control** — disable broadcast on operational-secrecy items; they live in the database and on this map but never go on the wire
- **SQLite-backed persistence** — the picture survives a restart, a power cycle, a swap of the laptop, a swap of the IC

### 3. Compose with the rest of the xTAK suite, no glue required
...
- **Incident commanders** running an EOC, command post, or coordination cell from a laptop — get full TAK SA without distributing tablets to every section chief.
...
A full TAK map, chat, and SA hub in a browser tab. baseTAK turns every laptop in your EOC into a first-class TAK operator alongside ATAK, WinTAK, and iTAK. No TAK Server. No cloud. No tablets required. Self-hosted on any Linux box. The home base of the xTAK suite.
...
### Soul quote
> A marker placed from the web looks and behaves identically to one placed from WinTAK.
